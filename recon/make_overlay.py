...
should carry over into that job: the value codebook and LUT below, nearest-
...
def build_lut() -> np.ndarray:
    ...
    PROVISIONAL. spec.md section 15.1 still owns the real snow/freshness
    encoding; this is just enough to read a snow map. Colour and opacity both
    rise with snow cover. 0% keeps a faint but non-zero alpha so "measured
    snow-free" stays visually distinct from "no data" (spec.md section 5.4).
    """
    ...
    lut[CLOUD] = (150, 150, 155, 110)  # grey - must not read as 0% snow
...
def main() -> int:
    ...
    sidecar = {
        ...
        "note": (
            "Generated by recon/make_overlay.py. Colour ramp is provisional - "
            "see docs/spec.md section 15.1."
        ),
    }
